...
class Part3():
    def __init__(self):
        ...
        self._fakeOdom = fakeOdom.fakeOdom()
    ...
    def initSubscriptions(self):
        ...
        def cmdCallback(reading):
                linearVel = reading.linear.x
                angularVel = reading.angular.z
                self._fakeOdom.simulate(linearVel,angularVel)
                
                
        rospy.Subscriber("commands", Twist, cmdCallback)
        # subscribe to transformation updates
        self._odoListener = tf.TransformListener() # listen to tf
        rospy.loginfo("Subscribed to command frames")
    ...
    def update(self, trans, rot):
        ...
        self._pFilter.receiveOdom(self._fakeOdom.getPosition())
